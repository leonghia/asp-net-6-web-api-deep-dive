...
public class CourseController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<CourseDto>>> GetCoursesForAuthor([FromRoute] Guid authorId)
    {
    ...
    public async Task<ActionResult<CourseDto>> GetCourseForAuthor([FromRoute] Guid authorId, [FromRoute] Guid courseId)
    {
        if (!await _courseLibraryRepository.AuthorExistsAsync(authorId))
        {
            return NotFound();
    ...
    public async Task<ActionResult<CourseDto>> CreateCourseForAuthor(
            [FromRoute] Guid authorId, [FromBody] CourseForCreationDto course)
    {
        if (!await _courseLibraryRepository.AuthorExistsAsync(authorId))
        {
            return NotFound();
    ...
    [HttpPut("{courseId}")]
    public async Task<IActionResult> UpdateCourseForAuthor([FromRoute] Guid authorId,
    ...
    [HttpDelete("{courseId}")]
    public async Task<ActionResult> DeleteCourseForAuthor([FromRoute] Guid authorId, [FromRoute] Guid courseId)
    {
        if (!await _courseLibraryRepository.AuthorExistsAsync(authorId))
        {
            return NotFound();
